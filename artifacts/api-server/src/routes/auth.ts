...
router.post("/auth/setup", async (req, res) => {
  try {
    ...
    const [user] = await db.insert(usersTable).values({
      username, email, passwordHash, role: "admin", status: "active"
    }).returning();
...
// POST /api/auth/register — contributor registration (starts as pending)
router.post("/auth/register", async (req, res) => {
  ...
  try {
    ...
    const passwordHash = await bcrypt.hash(password, 12);
    const [user] = await db.insert(usersTable).values({
      username, email, passwordHash, role: "contributor", status: "pending"
    }).returning();
    const token = signToken({ id: user.id, email: user.email, role: "contributor" });
    res.status(201).json({
      token,
